...
// A função iniciaJogo é automaticamente invocada quando o documento HTML tiver sido completamente carregado pelo browser.
window.onload = iniciaJogo;

/** 
 * Função que chama as funções necessárias para inicializar o jogo 
 */
function iniciaJogo(){
...
function criaEventListeners() {
    ...
    $(".escolheVolumeMusica").on("input", ".volume", function(ajustaVolume){  //Vai buscar pelas classes 
        volumeMusica = $(ajustaVolume.currentTarget).val(); // pega o valor obtido depois de mexer no botão
        localStorage.setItem("volumeMusica", volumeMusica/100); // mete o volume no local storage para ser usado pela função play no outro documento
    });
...
function defineDificuldade(dificuldade){
    ...
    else if(dificuldade == "Medio"){ //Acabar este
        let listaTabuleiros = 
        [
            [
 ...
 */
 ...
 function encontraPokemon(l,c){
    if (tabuleiro[l][c] == 'pokemon'){
        ...
        // localStorage.setItem("listaOponentes", JSON.stringify(listaOponentes))
 ...
 * @param {number} c, indica a coluna da celula que vai ser apagada do tabuleiro */
 ...
 function terminaJogo(l,c){
    if (tabuleiro[l][c] == 'casa' || tabuleiro[l][c] == 'casa_2'|| tabuleiro[l][c] == 'casa_3'){
        ...
        document.getElementById("alerta2").style.display = "block"; // POR NUMA CONSTANTE 
        paraTemporizador();
    }
}
